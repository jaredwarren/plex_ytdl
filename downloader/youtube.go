...
type Downloader interface {
	...
	DownloadVideo(videoID string) (string, *youtube.Video, error)
...
func (d *YoutubeDownloader) DownloadVideo(videoID string) (string, *youtube.Video, error) {
	client := youtube.Client{}
	...
	sEnc := fs.CleanFile(video.Title)
	fileName := filepath.Join(viper.GetString("player.song_root"), fmt.Sprintf("%s%s", sEnc, ext))

	d.Logger.Info("downloading video", log.Any("title", video.Title), log.Any("file", fileName))
	...
	if _, err := os.Stat(fileName); errors.Is(err, os.ErrNotExist) {
		...
		d.Logger.Warn("file already exists", log.Any("title", video.Title), log.Any("file", fileName))
	}
